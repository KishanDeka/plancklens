"""Parameter file for lensing reconstruction on a idealized, full-sky simulation library.
    ...
    The CMB simulations are the lensed CMB simulations obtained from taylens, together with homogeneous, Gaussian noise maps.

    The CMB simulations are located on "cmb_exec/outputs"
...
sys.path.append('plancklens/mylib')
import taylenslib
...
nsims = 100  # Total number of simulations to consider.
...
cl_unl = utils.camb_clfile(os.path.join(cls_path, 'ps_out.txt'))
cl_unl['bb'] *= 0.
cl_len = utils.camb_clfile(os.path.join(cls_path, 'ps_lns_theory.txt'))
#: Fiducial unlensed and lensed power spectra used for the analysis.

cl_weight = utils.camb_clfile(os.path.join(cls_path, 'ps_out.txt'))
cl_weight['bb'] *= 0.
#: CMB spectra entering the QE weights (the spectra multplying the inverse-variance filtered maps in the QE legs)

sims = maps_utils.sim_lib_shuffle(taylenslib.taylens_sims(fwhm,fwhm), {idx: nsims if idx == -1 else idx for idx in range(-1, nsims)})
...
ss_dict = { k : v for k, v in zip( np.concatenate( [ range(i*20, (i+1)*20) for i in range(0,5) ] ),
                    np.concatenate( [ np.roll( range(i*20, (i+1)*20), -1 ) for i in range(0,5) ] ) ) }
# Shuffling dictionary used for qlms_ds. This remap all sim. indices to the data maps:
#ds_dict = { k : -1 for k in range(100)}
...
mc_sims_bias = np.arange(20) #: The mean-field will be calculated from these simulations.
mc_sims_var  = np.arange(20, 100) #: The covariance matrix will be calculated from these simulations
